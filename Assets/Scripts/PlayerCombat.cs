...
public class PlayerCombat : Combat
{
    ...
    public void RangedAttack(GameObject player, GameObject poisonArrow, GameObject[] enemies, string orientation, float holdTimeDelta)
    {
        float bowHitRange = 1;
        float dist = 0;
        if (holdTimeDelta >= 1)
        ...
        GameObject arrow = Instantiate(poisonArrow, player.transform.position, Quaternion.identity);
        float x = 0;
        float y = 0;
        switch (orientation)
        {
